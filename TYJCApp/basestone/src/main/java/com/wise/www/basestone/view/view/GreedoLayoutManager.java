...
public class GreedoLayoutManager extends RecyclerView.LayoutManager {
    ...
    public GreedoLayoutManager setHorizontalSpace(int horizontalSpace) {
        this.horizontalSpace = horizontalSpace;
        mSizeCalculator.setHorizontalSpace(horizontalSpace);
        return this;
    }

    public GreedoLayoutManager setVerticalSpace(int verticalSpace) {
        this.verticalSpace = verticalSpace;
        mSizeCalculator.setVerticalSpace(verticalSpace);
        return this;
    }

    public GreedoLayoutManager setSpace(int space) {
        this.verticalSpace = space;
        this.horizontalSpace = space;
        mSizeCalculator.setHorizontalSpace(space);
        mSizeCalculator.setVerticalSpace(space);
        return this;
    }
    ...
    public GreedoLayoutManager(GreedoLayoutSizeCalculator.SizeCalculatorDelegate sizeCalculatorDelegate, int maxRowHeight) {
    ...
    public void setContentWidth(int contentWidth) {
        mSizeCalculator.setContentWidth(contentWidth);
    }
...
//        if(mFirstVisibleRow==0)newFirstVisiblePosition=0;
        ...
        while (nextPosition >= 0 && nextPosition < state.getItemCount()) {
            ...
            if (nextPosition == getItemCount()) {
                measureChildWithMargins(view, 0, 0);
                mFooterViewSize = new Size(view.getMeasuredWidth(), view.getMeasuredHeight());
            }
            ...
            if ((leftOffset + viewSize.getWidth()) > getContentWidth()) {
                ...
                topOffset += previousViewSize.getHeight() + verticalSpace;
            }
            ...
            leftOffset += (viewSize.getWidth()+horizontalSpace);
            nextPosition++;
        }
    ...
    public int getTotalHeight(int position) {
        return mSizeCalculator.getTotalHeight(position);
    }
    ...
    public int getLastRowCount() {
        return mSizeCalculator.getLastRowCount();
    }

    public int getLastRowHeight() {
        return mSizeCalculator.getLastRowHeight();
    }

    public int rowForChildPosition(int position) {
        int offset = 0;
        if (mIsFirstViewHeader && position == HEADER_POSITION) {
    ...
    public int firstChildPositionForRow(int row) {
        int offset = 0;
    ...
    public int scrollVerticallyBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (dy > 0) {
            boolean isLastChildVisible = (mFirstVisiblePosition + getChildCount()) >= state.getItemCount();
        ...
        //     If returned value does not match original delta (passed in),
        //     RecyclerView will draw an edge effect.
        return scrolled;
    }
